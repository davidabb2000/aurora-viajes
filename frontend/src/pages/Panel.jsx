  ...
  return (
    <main className="mx-auto w-[92%] max-w-275 flex-1 py-12 sm:py-16">
      ...
      <section className="vidrio mt-8 overflow-x-auto rounded-3xl p-6">
        ...
        <input value={busqueda} onChange={(evento) => setBusqueda(evento.target.value)} placeholder="Buscar por vuelo, aerolínea, origen o destino" className="mt-5 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 text-sm" />
        {vuelos.length === 0 ? (
        ...
        <form onSubmit={guardar} className="vidrio mt-8 rounded-3xl p-6">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[["numeroVuelo", "Número de vuelo"], ["aerolinea", "Aerolínea"], ["avion", "Avión"], ["origen", "Origen"], ["destino", "Destino"], ["puerta", "Puerta"], ["terminal", "Terminal"]].map(([campo, etiqueta]) => (
              <label key={campo} className="text-sm font-semibold text-texto">{etiqueta}<input required={!["puerta", "terminal"].includes(campo)} name={campo} value={(editando || nuevo)[campo]} onChange={(evento) => actualizarCampo(evento, editando ? setEditando : setNuevo)} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
            ))}
            <label className="text-sm font-semibold text-texto">Salida<input required type="datetime-local" name="fechaSalida" value={(editando || nuevo).fechaSalida} onChange={(evento) => actualizarCampo(evento, editando ? setEditando : setNuevo)} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
            <label className="text-sm font-semibold text-texto">Llegada<input required type="datetime-local" name="fechaLlegada" value={(editando || nuevo).fechaLlegada} onChange={(evento) => actualizarCampo(evento, editando ? setEditando : setNuevo)} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
            <label className="text-sm font-semibold text-texto">Capacidad máxima<input required type="number" min="1" max="1000" name="capacidadMaxima" value={(editando || nuevo).capacidadMaxima} onChange={(evento) => actualizarCampo(evento, editando ? setEditando : setNuevo)} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
            <label className="text-sm font-semibold text-texto">Estado<select name="estado" value={(editando || nuevo).estado} onChange={(evento) => actualizarCampo(evento, editando ? setEditando : setNuevo)} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal"><option value="programado">Programado</option><option value="abordando">Abordando</option><option value="en_vuelo">En vuelo</option><option value="aterrizado">Aterrizado</option><option value="cancelado">Cancelado</option></select></label>
          </div>
...
function PaquetesPanel({ paquetes, destinos, hoteles, excursiones, onCrearHotel, onCrearExcursion, onCrearPaquete, onEliminarPaquete }) {
    ...
    <main className="mx-auto w-[92%] max-w-275 flex-1 py-12 sm:py-16">
      ...
      <section className="vidrio mt-8 rounded-3xl p-6">
        ...
        <form onSubmit={(evento) => enviar(evento, "paquete")} className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <input required placeholder="Nombre de la reserva" value={paquete.nombre} onChange={(evento) => setPaquete({ ...paquete, nombre: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" />
          <select required value={paquete.destinoId} onChange={(evento) => { const destino = ubicaciones.find((item) => item.id === Number(evento.target.value)); setPaquete({ ...paquete, destinoId: evento.target.value, vuelo: { ...paquete.vuelo, destino: destino?.nombre || "" } }); }} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option value="">Destino</option>{ubicaciones.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select>
          <div className="vidrio-sutil rounded-2xl p-3 sm:col-span-2 lg:col-span-3"><p className="text-sm font-semibold text-texto">Configurar vuelo de esta reserva</p><p className="mt-1 text-xs text-texto-suave">El número, la puerta, la terminal y la capacidad se asignan automáticamente.</p><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><select required value={paquete.vuelo.aerolinea} onChange={(evento) => setPaquete({ ...paquete, vuelo: { ...paquete.vuelo, aerolinea: evento.target.value } })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option>Aurora Airlines</option><option>Avianca</option><option>LATAM</option><option>Copa Airlines</option><option>Iberia</option></select><select required value={paquete.vuelo.avion} onChange={(evento) => { const capacidades = { "Airbus A320": 180, "Airbus A330": 300, "Boeing 737": 189, "Boeing 787": 330, "Embraer E195": 132 }; setPaquete({ ...paquete, vuelo: { ...paquete.vuelo, avion: evento.target.value, capacidadMaxima: capacidades[evento.target.value] } }); }} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option>Airbus A320</option><option>Airbus A330</option><option>Boeing 737</option><option>Boeing 787</option><option>Embraer E195</option></select><select required value={paquete.vuelo.origen} onChange={(evento) => actualizarRuta("origen", evento.target.value)} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option value="">Origen</option><option>Bogotá</option><option>Medellín</option><option>Cali</option><option>Cartagena</option><option>Barranquilla</option><option>Lima</option><option>Madrid</option></select><input readOnly placeholder="Destino seleccionado" value={paquete.vuelo.destino} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /><select required value={paquete.vuelo.terminal} onChange={(evento) => setPaquete({ ...paquete, vuelo: { ...paquete.vuelo, terminal: evento.target.value } })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option value="">Terminal</option>{opcionesAeropuerto(paquete.vuelo.origen).terminales.map((item) => <option key={item}>{item}</option>)}</select><select required value={paquete.vuelo.puerta} onChange={(evento) => setPaquete({ ...paquete, vuelo: { ...paquete.vuelo, puerta: evento.target.value } })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option value="">Puerta</option>{opcionesAeropuerto(paquete.vuelo.origen).puertas.map((item) => <option key={item}>{item}</option>)}</select><input readOnly placeholder="Capacidad automática" value={`${paquete.vuelo.capacidadMaxima} pasajeros`} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /><input required type="datetime-local" value={paquete.vuelo.fechaSalida} onChange={(evento) => setPaquete({ ...paquete, fechaSalida: evento.target.value.slice(0, 10), vuelo: { ...paquete.vuelo, fechaSalida: evento.target.value } })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /><input required type="datetime-local" value={paquete.vuelo.fechaLlegada} onChange={(evento) => setPaquete({ ...paquete, vuelo: { ...paquete.vuelo, fechaLlegada: evento.target.value } })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /></div></div>
          <div className="vidrio-sutil grid gap-3 rounded-2xl p-3 sm:col-span-2"><select required value={paquete.hotelId} onChange={(evento) => setPaquete({ ...paquete, hotelId: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option value="">Hotel según destino</option>{hoteles.filter((item) => item.activo && coincideConDestino(item)).map((item) => <option key={item.id} value={item.id}>{item.nombre} · {item.ciudad}, {item.pais}</option>)}</select><div className="grid gap-3 sm:grid-cols-2"><input readOnly value={paquete.fechaSalida} placeholder="Entrada del hotel: fecha de ida" className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /><input readOnly value={paquete.fechaRegreso} placeholder="Salida del hotel: fecha de regreso" className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /></div></div>
          <input required type="date" value={paquete.fechaSalida} onChange={(evento) => setPaquete({ ...paquete, fechaSalida: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" />
          <input required type="date" value={paquete.fechaRegreso} onChange={(evento) => setPaquete({ ...paquete, fechaRegreso: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" />
          <input required type="number" min="0" value={paquete.precioBase} onChange={(evento) => setPaquete({ ...paquete, precioBase: evento.target.value })} placeholder="Precio por pasajero" className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" />
          <select multiple value={paquete.excursionIds} onChange={(evento) => setPaquete({ ...paquete, excursionIds: [...evento.target.selectedOptions].map((option) => option.value) })} className="min-h-24 rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option disabled>Excursiones según destino</option>{excursiones.filter((item) => item.activo && coincideConDestino(item)).map((item) => <option key={item.id} value={item.id}>{item.nombre} · {item.ciudad}, {item.pais}</option>)}</select>
          <button type="submit" className="rounded-xl boton-tinta text-white px-4 py-2 font-semibold">Publicar reserva</button>
        </form>
      </section>
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        <form onSubmit={(evento) => enviar(evento, "hotel")} className="vidrio rounded-3xl p-6"><h2 className="font-display text-2xl font-bold text-primario">Agregar hotel</h2><div className="mt-4 grid gap-3"><input required placeholder="Nombre" value={hotel.nombre} onChange={(evento) => setHotel({ ...hotel, nombre: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /><select required value={hotel.ubicacion} onChange={(evento) => setHotel({ ...hotel, ubicacion: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option value="">Ubicación de la reserva</option>{ubicaciones.map((destino) => <option key={destino.id} value={destino.nombre}>{destino.nombre}</option>)}</select><input required type="number" min="1" max="5" placeholder="Estrellas (1 a 5)" value={hotel.estrellas} onChange={(evento) => setHotel({ ...hotel, estrellas: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /><input required type="number" min="0" placeholder="Precio por noche" value={hotel.precioNoche} onChange={(evento) => setHotel({ ...hotel, precioNoche: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /></div><button className="mt-4 rounded-xl boton-tinta text-white px-4 py-2 font-semibold">Guardar hotel</button></form>
        <form onSubmit={(evento) => enviar(evento, "excursion")} className="vidrio rounded-3xl p-6"><h2 className="font-display text-2xl font-bold text-primario">Agregar excursión</h2><div className="mt-4 grid gap-3"><input required placeholder="Nombre" value={excursion.nombre} onChange={(evento) => setExcursion({ ...excursion, nombre: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /><select required value={excursion.ubicacion} onChange={(evento) => setExcursion({ ...excursion, ubicacion: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2"><option value="">Ubicación de la reserva</option>{ubicaciones.map((destino) => <option key={destino.id} value={destino.nombre}>{destino.nombre}</option>)}</select><input required type="number" min="1" max="48" placeholder="Duración en horas (1 a 48)" value={excursion.duracionHoras} onChange={(evento) => setExcursion({ ...excursion, duracionHoras: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /><input required type="number" min="0" placeholder="Precio" value={excursion.precio} onChange={(evento) => setExcursion({ ...excursion, precio: evento.target.value })} className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2" /></div><button className="mt-4 rounded-xl boton-tinta text-white px-4 py-2 font-semibold">Guardar excursión</button></form>
      </div>
...
function Panel() {
  ...
  const modificarReserva = async (evento) => {
    ...
    try {
      await solicitar(`/reservas/${reservaEditando.id}`, {
        ...
        body: JSON.stringify({
          ...reservaEditando,
          pasajeros: Number(reservaEditando.pasajeros),
          // El servidor recalcula el precio con lo que se envíe: sin esto se perderían el hotel y las excursiones.
          hotelId: reservaEditando.hotelId || null,
          paqueteId: reservaEditando.paqueteId || null,
          excursionIds: (reservaEditando.excursiones || []).map((excursion) => excursion.id),
        }),
      });
  ...
  if (vista === "paquetes" && esAdmin) return <PaquetesPanel paquetes={paquetes} destinos={destinos} hoteles={hoteles} excursiones={excursiones} onCrearHotel={crearHotel} onCrearExcursion={crearExcursion} onCrearPaquete={crearPaquete} onEliminarPaquete={eliminarPaquete} />;
    ...
    return (
      <main className="mx-auto w-[92%] max-w-275 flex-1 py-12 sm:py-16 [&_button]:cursor-pointer">
        ...
        <button
          ...
          className="mt-6 cursor-pointer rounded-xl boton-tinta text-white px-4 py-2 text-sm font-semibold"
        >
          Volver al panel
        </button>
        <section className="vidrio mt-8 overflow-x-auto rounded-3xl p-6">
            ...
            <table className="w-full min-w-175 text-left text-sm">
              ...
              <tbody>
                {mensajesContacto.map((item) => (
                  <tr key={item.id} className="border-b border-primario/12 align-top">
      ...
      <main className="mx-auto w-[92%] max-w-275 flex-1 py-12 sm:py-16 [&_button]:cursor-pointer">
        ...
        {cargando ? (
          ...
          <section className="vidrio mt-8 rounded-3xl p-6">
            ...
            <input value={busquedaReservas} onChange={(evento) => setBusquedaReservas(evento.target.value)} placeholder="Buscar por cliente, destino, vuelo o estado" className="mt-5 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 text-sm" />
            {reservas.length === 0 ? (
                  ...
                  >
                    ...
                    <details className="vidrio-sutil mt-5 rounded-2xl p-4">
                      ...
                      <div className="mt-4 grid gap-5 text-sm lg:grid-cols-2">
                        ...
                        <div className="border-t border-primario/12 pt-4 lg:col-span-2">
                    ...
                    <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-primario/12 pt-4">
                      {esPersonal && (
                        <label className="text-sm font-semibold text-texto">
                          ...
                          <select
                            ...
                            className="ml-2 cursor-pointer rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-2 py-1 font-normal"
                          >
                      ...
                      {rol === "cliente" && reserva.estadoPago !== "pagado" && reserva.estado !== "cancelada" && (
                        <button
                          type="button"
                          onClick={() => window.location.assign(`/reservas/pago/${reserva.id}`)}
                          className="cursor-pointer rounded-xl boton-tinta text-white px-3 py-2 text-sm font-semibold"
                        >
                          Ir al pago
                        </button>
                      )}
                    ...
                    {reservaEditando?.id === reserva.id && (
                      <form onSubmit={modificarReserva} className="mt-5 grid gap-4 border-t border-primario/12 pt-5 sm:grid-cols-2">
                        <label className="text-sm font-semibold text-texto sm:col-span-2">Destino<input required name="destino" value={reservaEditando.destino} onChange={(evento) => setReservaEditando({ ...reservaEditando, destino: evento.target.value })} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
                        <label className="text-sm font-semibold text-texto sm:col-span-2">Vuelo<select required name="vueloId" value={reservaEditando.vueloId || ""} onChange={(evento) => { const vuelo = vuelos.find((item) => item.id === Number(evento.target.value)); setReservaEditando({ ...reservaEditando, vueloId: Number(evento.target.value), origen: vuelo?.origen || "" }); }} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal"><option value="">Selecciona un vuelo</option>{vuelos.map((vuelo) => <option key={vuelo.id} value={vuelo.id}>{vuelo.numeroVuelo} · {vuelo.origen} → {vuelo.destino} · {new Date(vuelo.fechaSalida).toLocaleString("es-CO")}</option>)}</select></label>
                        <label className="text-sm font-semibold text-texto">Fecha de salida<input required type="date" name="fechaSalida" value={reservaEditando.fechaSalida} onChange={(evento) => setReservaEditando({ ...reservaEditando, fechaSalida: evento.target.value })} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
                        <label className="text-sm font-semibold text-texto">Fecha de regreso<input required type="date" name="fechaRegreso" value={reservaEditando.fechaRegreso} onChange={(evento) => setReservaEditando({ ...reservaEditando, fechaRegreso: evento.target.value })} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
                        <label className="text-sm font-semibold text-texto">Pasajeros<input required type="number" min="1" max="9" name="pasajeros" value={reservaEditando.pasajeros} onChange={(evento) => setReservaEditando({ ...reservaEditando, pasajeros: evento.target.value })} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
                        <label className="text-sm font-semibold text-texto">Teléfono de contacto<input required pattern="[0-9]{7,10}" maxLength="10" name="telefonoContacto" value={reservaEditando.telefonoContacto} onChange={(evento) => setReservaEditando({ ...reservaEditando, telefonoContacto: evento.target.value })} className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
                        <label className="text-sm font-semibold text-texto sm:col-span-2">Notas<textarea maxLength="300" name="notas" value={reservaEditando.notas || ""} onChange={(evento) => setReservaEditando({ ...reservaEditando, notas: evento.target.value })} rows="3" className="mt-2 w-full resize-y rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal" /></label>
                        <div className="flex gap-3 sm:col-span-2"><button type="submit" className="cursor-pointer rounded-xl boton-tinta text-white px-4 py-2 text-sm font-semibold">Guardar cambios</button><button type="button" onClick={() => setReservaEditando(null)} className="cursor-pointer vidrio rounded-xl text-primario transition hover:-translate-y-0.5 hover:bg-white/85 px-4 py-2 text-sm font-semibold">Cancelar</button></div>
                      </form>
                    )}
                  </article>
                ))}
    ...
    <main className="mx-auto w-[92%] max-w-275 flex-1 py-12 sm:py-16 [&_button]:cursor-pointer">
      ...
      {cargando ? (
          ...
          <table className="mt-5 w-full min-w-175 text-left text-sm">
            ...
            <tbody>
              {usuarios.map((usuario) => (
                <tr key={usuario.id} className="border-b border-primario/12">
                  <td className="py-3">
        ...
        >
            ...
            {[
              ...
              <label key={campo} className="text-sm font-semibold text-texto">
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-3 py-2 font-normal"
                />
              </label>
            ))}
            <label className="text-sm font-semibold text-texto">
          ...
          <button
            type="submit"
            className="mt-5 rounded-xl boton-tinta text-white px-5 py-2.5 font-semibold"
          >
            Crear usuario
          </button>
        </form>
          ...
          {reservas.length === 0 ? (
            ...
            <div className="mt-5 overflow-x-auto">
              <table className="w-full min-w-175 text-left text-sm">
                ...
                <tbody>
                  {reservas.map((reserva) => (
                    <tr key={reserva.id} className="border-b border-primario/12">
                      <td className="py-3">
                        {esPersonal ? reserva.cliente : reserva.destino}
                      </td>
                      ...
                      {esPersonal && (
                        <td>
                          <select
                            ...
                            className="rounded-xl border border-primario/12 bg-white/70 shadow-sm shadow-primario/5 outline-none backdrop-blur-sm transition focus:border-primario-suave focus:bg-white/90 focus:ring-3 focus:ring-primario-suave/20 px-2 py-1"
                          >
      ...
      {reservaDetalle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-primario-oscuro/50 p-4 backdrop-blur-md" role="presentation" onClick={() => setReservaDetalle(null)}>
          <section className="vidrio-solido max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl p-6 sm:p-8" role="dialog" aria-modal="true" aria-labelledby="detalle-reserva-titulo" onClick={(evento) => evento.stopPropagation()}>
            <div className="flex items-start justify-between gap-4 border-b border-primario/12 pb-4">
              <div>
            ...
            <div className="mt-5 grid gap-5 text-sm sm:grid-cols-2">
              ...
              <div className="vidrio-sutil rounded-2xl p-4">
                <h3 className="font-semibold text-primario">Excursiones</h3>
                {reservaDetalle.paquete?.excursiones?.length ? <ul className="mt-3 divide-y divide-primario/10">{reservaDetalle.paquete.excursiones.map((excursion) => <li key={excursion.id} className="py-2 first:pt-0 last:pb-0"><strong className="text-texto">{excursion.nombre}</strong><span className="block text-texto-suave">{excursion.ciudad}, {excursion.pais} · {excursion.duracionHoras} horas</span></li>)}</ul> : <p className="mt-3 text-texto-suave">Sin excursiones asociadas.</p>}
              </div>
            </div>
            <div className="mt-5 border-t border-primario/12 pt-4 text-sm"><p><span className="text-texto-suave">Contacto: </span><strong>{reservaDetalle.telefonoContacto || "No registrado"}</strong></p><p className="mt-2"><span className="text-texto-suave">Pago: </span><strong className="capitalize">{reservaDetalle.estadoPago || "pendiente"}</strong>{reservaDetalle.metodoPago && ` · ${reservaDetalle.metodoPago}`}</p>{reservaDetalle.notas && <p className="vidrio-sutil mt-3 rounded-2xl p-3"><strong>Notas:</strong> {reservaDetalle.notas}</p>}</div>
          </section>
        </div>
      )}
      {mensaje && <p className="mt-4 text-sm text-primario">{mensaje}</p>}
